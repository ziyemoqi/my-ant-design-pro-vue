...
<script>
import { classTree, add, updateClass } from '@/api/mall/mallGoodClass'
...
export default {
  ...
  methods: {
    ...
    edit(record) {
      ...
      this.mallGoodClassId = record != null ? record.mallGoodClassId.toString() : null
      this.$nextTick(() => {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (that.addFlag) {
            ...
            let mallGoodClassId = that.mallGoodClassId
            let editData = {
              ...formData,
              mallGoodClassId
            }
            updateClass(editData)
              .then(res => {
                if (res.code === 200) {
                  that.$message.success('操作成功!')
                  that.loadTreeData()
